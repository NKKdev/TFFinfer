...
#pragma unroll
            for (int d = thread_x; d < dim0; d += WARP_SIZE) {
                dst_ptr[row_index * dim1 * dim0 + d] = src_ptr[row_index * dim1 * dim0 + d] * rms;
            }
        }
    }
    ...
    template<typename T>
    class Norm<T,
                core::device::GPUTag> : public base::OPCreatorBase<Norm<T, core::device::GPUTag>, T,
                core::device::GPUTag> {
    public:
    ...
    //
    template<typename T>
    void tff::kernel::Norm<T, core::device::GPUTag>::compute(
        std::shared_ptr<tff::core::global::ParamBaseObject> &para_ptr) {
        auto type = kernel::base::get_param_value<core::graph::TFFNormType>(NormBuilder::Params::NormType, para_ptr);
        auto eps = kernel::base::get_param_value<float>(NormBuilder::Params::Epsilon, para_ptr);
        auto weight = kernel::base::get_param_value<std::shared_ptr<tff::core::memory::Tensor> >(
            MulBuilder::Params::Weight, para_ptr);
        auto x = kernel::base::get_param_value<std::shared_ptr<tff::core::memory::Tensor> >(
            NormBuilder::Params::In, para_ptr);
        auto output_tensor = kernel::base::get_param_value<std::shared_ptr<tff::core::memory::Tensor> >(
            NormBuilder::Params::Out, para_ptr);
        auto stream = kernel::base::get_param_value<std::shared_ptr<core::device::DeviceStream> >(
            kernel::builder::OpParamBuilderBase<NormBuilder>::CommonParams::Stream, para_ptr);
        if (x == nullptr || output_tensor == nullptr) {
            return;
        }
        //
        switch (type) {
            case core::graph::TFFNormType::TFF_NORM_RMS:
    ...
    template<typename T>
    class NormW<T,
                core::device::GPUTag> : public base::OPCreatorBase<NormW<T, core::device::GPUTag>, T,
                core::device::GPUTag> {
    public:
        static void compute(std::shared_ptr<tff::core::global::ParamBaseObject> &para_ptr);

        inline static core::graph::TffOpType op_type() {
            return core::graph::TffOpType::TFF_OP_RMS_NORM_W;
        }
    };

    //
    template<typename T>
    void tff::kernel::NormW<T, core::device::GPUTag>::compute(
        std::shared_ptr<tff::core::global::ParamBaseObject> &para_ptr) {
        auto type = kernel::base::get_param_value<core::graph::TFFNormType>(NormBuilder::Params::NormType, para_ptr);
        auto eps = kernel::base::get_param_value<float>(NormBuilder::Params::Epsilon, para_ptr);
        auto weight = kernel::base::get_param_value<std::shared_ptr<tff::core::memory::Tensor> >(
    ...
    template class tff::kernel::NormW<float, core::device::GPUTag>;
    template class tff::kernel::NormW<int32_t, core::device::GPUTag>;
    template class tff::kernel::NormW<int64_t, core::device::GPUTag>;
    REGISTER_OP_OBJECT_DEVICE(NormW, float, core::device::GPUTag);
    REGISTER_OP_OBJECT_DEVICE(NormW, int32_t, core::device::GPUTag);
    REGISTER_OP_OBJECT_DEVICE(NormW, int64_t, core::device::GPUTag);

    template class tff::kernel::Norm<float, core::device::GPUTag>;
    template class tff::kernel::Norm<int32_t, core::device::GPUTag>;
    template class tff::kernel::Norm<int64_t, core::device::GPUTag>;
    REGISTER_OP_OBJECT_DEVICE(Norm, float, core::device::GPUTag);
    REGISTER_OP_OBJECT_DEVICE(Norm, int32_t, core::device::GPUTag);
    REGISTER_OP_OBJECT_DEVICE(Norm, int64_t, core::device::GPUTag);
}
