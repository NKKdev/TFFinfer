...
namespace tff::kernel {
    ...
    static void memcpy_kernel_cuda(
        ...
        auto allocator = src->get_allocator();
    ...
    template<typename T>
    class MemCpy<T,
                core::device::CPUTag> : public base::OPCreatorBase<MemCpy<T, core::device::CPUTag>, T,
                core::device::CPUTag> {
    public:
    ...
    //
    template<typename T>
    void tff::kernel::MemCpy<T, core::device::CPUTag>::compute(
        ...
        auto name = kernel::base::get_param_value<std::string>(
            kernel::builder::OpParamBuilderBase<MemCpyBuilder>::CommonParams::Name, para_ptr);
        tff::log::Logger::info("op_name:%s input_tensor:%p,output tensor : %p",
            name.c_str(),input_tensor.get(), output_tensors.get());
#endif
    ...
    template class tff::kernel::MemCpy<float, core::device::CPUTag>;
    template class tff::kernel::MemCpy<double, core::device::CPUTag>;
    template class tff::kernel::MemCpy<int32_t, core::device::CPUTag>;
    template class tff::kernel::MemCpy<int64_t, core::device::CPUTag>;
    template class tff::kernel::MemCpy<Q8_0, core::device::CPUTag>;
    REGISTER_OP_OBJECT_DEVICE(MemCpy, float, core::device::CPUTag);

    REGISTER_OP_OBJECT_DEVICE(MemCpy, double, core::device::CPUTag);

    REGISTER_OP_OBJECT_DEVICE(MemCpy, int32_t, core::device::CPUTag);

    REGISTER_OP_OBJECT_DEVICE(MemCpy, int64_t, core::device::CPUTag);

    REGISTER_OP_OBJECT_DEVICE(MemCpy, Q8_0, core::device::CPUTag);
}
