...
//     const int BLOCK_DIM_M, const int BLOCK_DIM_N, const int BLOCK_DIM_K, const int PAD_SIZE>
// __device__ __forceinline__ void compute_tile_attention_gemm_with_rope_opt(const int k_size, const int thread_x, const int warp_id,
...
//
//                 c_reg[mm * VEC_DIM_N + nn] += (a_reg[0] * a_cos_sin_coef.x - a_reg[1] * a_cos_sin_coef.y) * (b_reg[0] * b_cos_sin_coef.x - b_reg[1] * b_cos_sin_coef.y)
//                 + (a_reg[0] * a_cos_sin_coef.y + a_reg[1] * a_cos_sin_coef.x) * (b_reg[0] * b_cos_sin_coef.y + b_reg[1] * b_cos_sin_coef.x);
//             }
//         }
//     }
// }
...
// template<typename T, const int VEC_DIM_M, const int VEC_DIM_N,
//     const int BLOCK_DIM_M, const int BLOCK_DIM_N, const int BLOCK_DIM_K, const int PAD_SIZE>
// __device__ __forceinline__ void compute_tile_attention_gemm_with_rope(const int k_size, const int thread_x, const int warp_id,
//                                                       const int start_m, const int start_n,
//                                                       T *a_sm, T *b_sm, float *cos_sin_sm,
//                                                       float *c_reg) {
//     auto *q_cos_sin_table_ptr = reinterpret_cast<float2 *>(cos_sin_sm + start_m * BLOCK_DIM_K);
...
// __device__ __forceinline__ void compute_softmax_pv(const int N, const int v_ld, const int start_m, const int start_n, const int start_d,
//                                    const int thread_x, const int warp_id,
...
//
//     flash_attention<T, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE, ACTUAL_ELEMENTS_PER_LOAD, rope_flag><<<
//             grid
//             , block>>>(
//                 M, N, D,
//                 D, D, D,
...
//     }
//
//     flash_attention<T, 0>(batch, m, n, k, num_q_heads, num_kv_heads, a_mat, b_mat, c_mat, cos_table, sin_table);
//
// #else
//     int dim = 1024;
//     int m = dim;
