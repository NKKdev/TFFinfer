...
template<const int WARP_SIZE, const int BLOCK_SIZE, const int WARP_NUM_PER_BLOCK, const int MAX_RANGE_VALUE>
...
#pragma unroll
    ...
    // if (start_dst_row == 156 && start_dst_col == 10 && lane_id == 19) {
    //     printf("d: %f\n", d);
    //     printf("x: %f\n", x);
    //     printf("x / d: %f\n", x / d);
    //     printf("roundf(x / d): %f\n", roundf(x / d));
    //     printf("static_cast<int32_t>(roundf(x / d)): %d\n", static_cast<int32_t>(roundf(x / d)));
...
void quant_q_8_0_1d(const int m, const int n, std::vector<float> &src, std::vector<float> &dst) {
    ...
    dim3 grid((n * m + BLOCK_SIZE - 1) / BLOCK_SIZE, 1, 1);
