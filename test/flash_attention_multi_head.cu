...
__global__ void flash_attention_cuda(const int M, const int N, const int D,
    ...
    const int num_warps = THREAD_BLOCK_SIZE / 32;
    ...
    int flip_flag = 0;
    load_tile_vec<T, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_K, PAD_SIZE>(
        q_ld, M, thread_x, warp_id, start_m, 0,
        q, &sm[0][0][0][0]);
    load_tile_vec<T, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE>(
...
template<typename T, const int VEC_DIM_M, const int VEC_DIM_N,
    const int VEC_DIM_K, const int BLOCK_DIM_M, const int BLOCK_DIM_N, const int BLOCK_DIM_K, const int PAD_SIZE>
__global__ void flash_attention_warp_opt_cuda(const int M, const int N, const int D,
                                     const int q_ld, const int k_ld, const int v_ld,
                                     const float scale,
                                     const int num_q_heads, const int num_kv_heads,
                                     const T *__restrict__ q_global,
    ...
    load_tile_vec<T, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_K, PAD_SIZE>(
        ...
        q, &sm[0][0][0][0]);
    load_tile_vec<T, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE>(
        k_ld, N, thread_x, warp_id, warp_id * BLOCK_DIM_N, 0,
        k, &sm[flip_flag][1][0][0]);
    __syncthreads();

    const int n_stage = (N + BLOCK_DIM_N - 1) / BLOCK_DIM_N;//8
    const int n_stage_per_warp = n_stage / num_warps;//1
...
void flash_attention_cpu_single_head(
    ...
    for (int m_start = 0; m_start < m; m_start += block_size_m) {
        ...
        for (int mm = 0; mm < block_size_m; ++mm) {
            for (int l = 0; l < k; ++l) {
                if (running_sum[mm] != 0) {
                    out_ptr[(mm + m_start) * k + l] /= running_sum[mm];
                }
            }
        }
    }
...
std::vector<float> flash_attention_cpu_gqa(
    ...
    for (int b = 0; b < batch; ++b) {
        for (int qh = 0; qh < num_q_heads; ++qh) {
            int kvh = qh / q_per_kv; // map query head to kv head
            ...
            flash_attention_cpu_single_head(
                m, n, head_dim,
                q_ptr, k_ptr, v_ptr,
                out_ptr
            );
...
void flash_attention(int batch,
    ...
    std::vector<float> cpu_result = flash_attention_cpu_gqa(batch, M, N, D, num_q_heads, num_kv_heads, q_mat, k_mat,
                                                            v_mat);
    // printf("cpu_result: \n");
    // for (int i = 0; i < M; i++) {
    //     for (int j = 0; j < D; j++) {
