    ...
    for (int j = 0; j < VEC_DIM_LD; ++j) {
        ...
        for (int kk = 0; kk < VEC_DIM_K / ELEMENTS_PER_LOAD; ++kk) {
            ...
            if (dim0_base < dim) {
                const int actual_load = min(ELEMENTS_PER_LOAD, ld - dim1);
                if (actual_load > 0) {
                    load_vec_async<T, half2>(&global_mem[dim0_base * ld + dim1], &sm[addr]);
                }
            } else {
                auto val_vec = reinterpret_cast<float4 *>(&sm[addr]);
                auto val_vec_ptr = reinterpret_cast<half2 *>(val_vec);
                half2 value[ELEMENTS_PER_LOAD / 2];
#pragma unroll
                for (int i = 0; i < ELEMENTS_PER_LOAD / 2; ++i) {
                    half2 val = val_vec_ptr[i];
                    value[i].x = 0;
                ...
                val_vec_ptr = reinterpret_cast<half2 *>(&value[0]);
            }
        }
    }
}
...
template<const int VEC_DIM_M, const int VEC_DIM_N, const int BLOCK_DIM_M, const int BLOCK_DIM_N, const int
    BLOCK_DIM_K, const int ELEMENTS_PER_LOAD, const int B, const int MBit, const int S>
__device__ __forceinline__ void compute_softmax_pv_shared(const int N, const int v_ld, const int start_m,
                                                          const int start_n,
                                                          const int start_d,
                                                          const int thread_x, const int warp_id,
                                                          float *max_value, float *sum_value, half2 *sm,
...
#pragma unroll
            ...
            output_ptr[output_index].y = __float2half(
                (__half2float(output_ptr[output_index].y) * old_sum * expf(old_max - new_max_value) +
                 acc.y * expf(current_max_value - new_max_value)) / new_sum_value);
            if (start_m == 0 && kk == 0 && warp_id == 0 && thread_x == 0) {
                printf("m: %d, k: %d, output_ptr.x: %lf,output_ptr.y: %lf \n",
                           (warp_id + mm * BLOCK_DIM_M / VEC_DIM_M), kk,
                           __half2float(output_ptr[output_index].x),
                           __half2float(output_ptr[output_index].y));
            }
...
__global__ void flash_attention(const int M, const int N, const int D,
    ...
    __shared__ half2 sm[2][shared_mem_block_size * 2];
    ...
    load_tile_vec_k<T_KV, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
        ...
        k_ptr, cos_sin_table, &sm[0][(flip_flag) * shared_mem_block_size]);

    load_tile_vec_v<T_KV, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
        v_ld, N, thread_x, warp_id, 0, 0,
        v_ptr, &sm[1][(flip_flag) * shared_mem_block_size]);
...
#pragma unroll
    ...
    for (int n = 0; n < n_stage; n++) {
        ...
        compute_tile_attention_gemm_with_rope<T_KV, VEC_DIM_M, VEC_DIM_N, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K,
            ELEMENTS_PER_LOAD, B, MBit, S>(
            N, k_size, thread_x, warp_id, start_m, n_start, q_sm, &sm[0][(flip_flag) * shared_mem_block_size],
            mask, scale, c_reg);
        ...
        if (next_n < N) {
            load_tile_vec_k<T_KV, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
                ...
                k_ptr, cos_sin_table, &sm[0][(1 - flip_flag) * shared_mem_block_size]);

            load_tile_vec_v<T_KV, VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
                v_ld, N, thread_x, warp_id, next_n, 0,
                v_ptr, &sm[1][(1 - flip_flag) * shared_mem_block_size]);
        }
        ...
        compute_softmax_pv<VEC_DIM_M, VEC_DIM_N, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
            ...
            &sum_value[0], &sm[1][(flip_flag) * shared_mem_block_size], &c_reg[0], output);
        flip_flag ^= 1;
    }
}
...
template<typename T_Q, typename T_KV, const int VEC_DIM_M, const int VEC_DIM_N,
    ...
    const int ELEMENTS_PER_LOAD,
    const int rope_flag, const int B, const int MBit, const int S, const int HEAD_NUM_PER_BLOCK>
__global__ void flash_attention_double_buffer_k_output_shared(const int M, const int N, const int D,
                                                              const int q_ld, const int k_ld, const int v_ld,
                                                              const float scale,
                                                              const int num_q_heads, const int num_kv_heads,
                                                              const T_Q *__restrict__ q_global,
    ...
    constexpr int output_sm_size = (BLOCK_DIM_K / 2 + PAD_SIZE) * (BLOCK_DIM_M);
    __shared__ half2 output_sm[output_sm_size];
#pragma unroll
    for (int mm = 0; mm < VEC_DIM_M; mm++) {
        for (int kk = 0; kk < 2; kk++) {
            output_sm[(warp_id + mm * BLOCK_DIM_M / VEC_DIM_M) * (BLOCK_DIM_K / 2 + PAD_SIZE) + thread_x + kk * BLOCK_DIM_K / 4].x = 0.0f;
            output_sm[(warp_id + mm * BLOCK_DIM_M / VEC_DIM_M) * (BLOCK_DIM_K / 2 + PAD_SIZE) + thread_x + 1 + kk * BLOCK_DIM_K / 4].x = 0.0f;
        }
    }
...
#pragma unroll
    ...
    for (int n = 0; n < n_stage; n++) {
        ...
        compute_softmax_pv_shared<VEC_DIM_M, VEC_DIM_N, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B,
            MBit, S>(
            N, v_ld, start_m, n_start, d, thread_x, warp_id, &max_value[0],
            &sum_value[0], &sm[(flip_flag) * shared_mem_block_size], &c_reg[0], output_sm);
        flip_flag ^= 1;
    }

    __syncthreads();

    const int store_warp_id = thread_id / 32;
    const int store_thread_x = thread_id % 32;
    const int vec_m = BLOCK_DIM_M / 8;
    float2 *output_val_ptr = reinterpret_cast<float2 *>(&output);
#pragma unroll
    for (int mm = 0; mm < vec_m; mm++) {
        int row_index = store_warp_id + mm * BLOCK_DIM_M / vec_m;
        if (row_index >= M) {
            continue;
        }
        const int vec_k = BLOCK_DIM_K / 2 / 32;
        for (int k = 0; k < vec_k; k++) {
            int col_index = store_thread_x + k * BLOCK_DIM_K / 2 / vec_k;
            if (col_index < D) {
                output_val_ptr[(start_m + row_index) * v_ld / 2 + col_index] =
                        __half22float2(output_sm[row_index * (BLOCK_DIM_K / 2 + PAD_SIZE) + col_index]);
                if (start_m == 0 && k == 0 && store_warp_id == 0 && store_thread_x == 0) {
                    printf("m: %d, k: %d, result.x: %lf,result.y: %lf \n",
                           start_m + row_index, col_index,
                           __half22float2(output_sm[row_index * (BLOCK_DIM_K / 2 + PAD_SIZE) + col_index]).x,
                           __half22float2(output_sm[row_index * (BLOCK_DIM_K / 2 + PAD_SIZE) + col_index]).y);
                }
            }
        }
    }
...
void flash_attention_double_buffer_k(int batch,
    ...
    flash_attention_double_buffer_k<T_Q, T_KV, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K,
                ELEMENTS_PER_LOAD, rope_flag, B, MBit, S, HEAD_NUM_PER_BLOCK><<<
            grid
            , block>>>(
                M, N, D,
