...
//     BLOCK_DIM_K,const int B, const int MBit, const int S>
...
//         //
//         for (int nn = 0; nn < valid_n; nn++) {
//             float c_reg_value = c_reg[mm * VEC_DIM_N + nn] - current_max_value;
//             c_reg[mm * VEC_DIM_N + nn] = expf(c_reg_value);
...
//         new_sum_value = current_sum_value * exp_local + old_sum * exp_old;
//
...
//                 (__half2float(output_val.x) * old_sum * exp_old +
//                  acc.x * exp_local) / new_sum_value);
//             output_val.y = __float2half(
//                 (__half2float(output_val.y) * old_sum * exp_old +
//                  acc.y * exp_local) / new_sum_value);
//
...
//         int sm_row = warp_id + j * (BLOCK_DIM_LD / VEC_DIM_LD);
//         for (int kk = 0; kk < VEC_DIM_K / 2; ++kk) {
...
//             float2 out = __half22float2(output_sm[row_base * (BLOCK_DIM_K / 2 + PAD_SIZE) + col_base]);
...
//     constexpr int VEC_DIM_M_CM = BLOCK_DIM_M / (THREAD_BLOCK / WARP_SIZE);
//     constexpr int VEC_DIM_N_CM = BLOCK_DIM_N / WARP_SIZE;
//     constexpr int VEC_DIM_K_CM = BLOCK_DIM_K / WARP_SIZE;
// #ifndef _ASYNC
//     const int ELEMENTS_PER_LOAD_SCALAR = 2;
// #endif
...
// #ifdef _DOUBER_BUFFER
//     __shared__ half2 sm[shared_mem_block_size * 2]; //2 * kv;(k, v共享一块shared mem)
// #else
//     __shared__ half2 sm[shared_mem_block_size * 2]; //2 * kv;(k, v不共享一块shared mem)
// #endif
...
//     init<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K>(cm_thread_x, cm_thread_y, &out_sm[0], 0);
//
//     int flip_flag = 0;
// #ifndef _ASYNC
//     //load q;
//     load_tile<T_Q, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//         q_stride, M, cm_thread_x, cm_thread_y, start_m, 0,
//         q_ptr, &q_sm[0]);
//     if constexpr (rope_flag == 1) {
//         rope<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//             q_ld, MAX_CTX, cm_thread_x, cm_thread_y, start_m, 0,
//             cos_sin_table, &q_sm[0]);//Q rope
//     }
// #ifdef _DOUBER_BUFFER
//     load_tile<T_KV, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//         kv_stride, N, cm_thread_x, cm_thread_y, 0, 0,
...
// #ifdef _DOUBER_BUFFER
//         rope<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//             q_ld, MAX_CTX, cm_thread_x, cm_thread_y, 0, 0,
//             cos_sin_table, &sm[(flip_flag) * shared_mem_block_size]);//K rope
// #endif
//
// #else
...
//
//         rope<VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
//             q_ld, MAX_CTX, ld_thread_x, ld_thread_y, start_m, 0,
...
//         //
// #ifndef _DOUBER_BUFFER
//         //
//         load_tile<T_KV, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//             kv_stride, N, cm_thread_x, cm_thread_y, n_start, 0,
//             k_ptr, &sm[(flip_flag) * shared_mem_block_size]);
//         if constexpr (rope_flag == 1) {
//             //
//             rope<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//                 q_ld, MAX_CTX, cm_thread_x, cm_thread_y, n_start, 0,
//                 cos_sin_table, &sm[(flip_flag) * shared_mem_block_size]);
//         }
//         load_tile<T_KV, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//             kv_stride, N, cm_thread_x, cm_thread_y, n_start, 0,
//             v_ptr, &sm[(flip_flag + 1) * shared_mem_block_size]);
//         __syncthreads();
// #endif
//
// #ifdef _ASYNC
...
//             //
//             load_tile<T_KV, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//                 kv_stride, N, cm_thread_x, cm_thread_y, next_n, 0,
//                 k_ptr, &sm[(1 - flip_flag) * shared_mem_block_size]);
//             if constexpr (rope_flag == 1) {
//                 //
//                 rope<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//                     q_ld, MAX_CTX, cm_thread_x, cm_thread_y, next_n, 0,
...
//                 //
//                 rope<VEC_DIM_M, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_K, ELEMENTS_PER_LOAD, B, MBit, S>(
//                     q_ld, MAX_CTX, ld_thread_x, ld_thread_y, next_n, 0,
//                     cos_sin_table, &sm[(1 - flip_flag) * shared_mem_block_size]);
//             }
// #endif
//         }
...
// #ifdef _DOUBER_BUFFER
//         load_tile<T_KV, VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_N, BLOCK_DIM_K, ELEMENTS_PER_LOAD_SCALAR, B, MBit, S>(
//             kv_stride, N, cm_thread_x, cm_thread_y, n_start, d,
...
//
// #ifdef _DOUBER_BUFFER
//         compute_softmax_pv<VEC_DIM_M_CM, VEC_DIM_N_CM, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, B,
//             MBit
//             , S>(
//             N, q_stride, start_m, n_start, d, cm_thread_x, cm_thread_y, &max_value[0],
//             &sum_value[0], &sm[(flip_flag) * shared_mem_block_size], scale, &c_reg[0], &out_sm[0]);
//         __syncthreads();
//         flip_flag ^= 1;
// #else
//         compute_softmax_pv<VEC_DIM_M_CM, VEC_DIM_N_CM, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, B,
...
//             , S>(
//             N, q_stride, start_m, n_start, d, cm_thread_x, cm_thread_y, &max_value[0],
//             &sum_value[0], &sm[(flip_flag + 1) * shared_mem_block_size], scale, &c_reg[0], &out_sm[0]);
//         __syncthreads();
// #endif
//     }
//
//     __syncthreads();
//     store<VEC_DIM_M_CM, VEC_DIM_K_CM, BLOCK_DIM_M, BLOCK_DIM_K, PAD_SIZE>(
...
//     printf("block x: %d, block y: %d\n", block.x, block.y);
//
//     flash_attention_double_buffer_k_out_sm<T_Q, T_KV, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_N,
//                 BLOCK_DIM_K,
