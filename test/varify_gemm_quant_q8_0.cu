...
//
// template<typename T>
// void varify_quant_q_8_0_gemm_nt(const int m, const int n, const int k, const int quant_k,
//                          std::vector<float> &a_mat,
//                          std::vector<float> &b_mat) {
//     std::vector<Q_8> a_quant;
//     a_quant.resize(m * quant_k);
//     load_tensor_raw("x_tensor.tff", a_quant.data());
...
//     b_quant.resize(n * quant_k);
//     load_tensor_raw("q_weight_quant.tff", b_quant.data());
...
//     printf("grid:%d, %d\n", grid.x, grid.y);
//     printf("block:%d, %d\n", block.x, block.y);
//     mat_mul_quant_q_8_0_double_buffer<T, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_M_DIM, BLOCK_N_DIM, BLOCK_K_DIM,
...
//                          quant_k, m, a_quant.data(), b_quant.data(), c_mat_cpu.data());
//     std::vector<float> c_mat_cpu_ggml;
//     c_mat_cpu_ggml.resize(m * n);
//     load_tensor_raw("Qcur-0_result.ggml", c_mat_cpu_ggml.data());
//     for (int i = 0; i < m; i++) {
//         for (int j = 0; j < n; j++) {
//             float delta = c_mat[i * n + j] - c_mat_cpu_ggml[i * n + j];
...
//
//
// int main6345(int argc, char *argv[]) {
...
//
//     varify_quant_q_8_0_gemm_nt<T>(n, m, k, quant_k, a_mat, b_mat);
