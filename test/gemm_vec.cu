...
#pragma unroll
    for (int j = 0; j < VEC_DIM_LD / ELEMENTS_PER_LOAD; j++) {
        int dim0 = start_m + thread_x * ELEMENTS_PER_LOAD + j * BLOCK_DIM_LD / VEC_DIM_LD;
        for (int kk = 0; kk < VEC_DIM_K; kk++) {
            int dim1 = k + thread_y + kk * BLOCK_DIM_K / VEC_DIM_K;
            T val[ELEMENTS_PER_LOAD] = {0.0f};
            if (dim1 < dim && dim0 < ld) {
                //val = __ldg(&global_mem[dim1 * ld + dim0]);
                if (dim0 % ELEMENTS_PER_LOAD != 0) {
                    load_vec<T>(&global_mem[dim1 * ld + dim0], val, dim0 % ELEMENTS_PER_LOAD);
                } else
                {
                    load_vec<T>(&global_mem[dim1 * ld + dim0], val, ELEMENTS_PER_LOAD);
                }
            }
#pragma unroll
            for (int i = 0; i < ELEMENTS_PER_LOAD; i++) {
                sm[(thread_y + kk * BLOCK_DIM_K / VEC_DIM_K) * (BLOCK_DIM_LD + PAD_SIZE) + thread_x * ELEMENTS_PER_LOAD
                   + j * BLOCK_DIM_LD / VEC_DIM_LD + i] = val[i];
            }
        }
    }
}
    ...
    for (int j = 0; j < VEC_DIM_LD; j++) {
        ...
        for (int kk = 0; kk < VEC_DIM_K; kk++) {
            ...
            if (dim0 < dim && dim1 < ld) {
                ...
                load_vec<T>(&global_mem[dim1 + dim0 * ld], val, ELEMENTS_PER_LOAD);
            }
#pragma unroll
            for (int i = 0; i < ELEMENTS_PER_LOAD; i++) {
                sm[(thread_x + kk * BLOCK_DIM_K / VEC_DIM_K) * (BLOCK_DIM_LD + PAD_SIZE) + thread_y * ELEMENTS_PER_LOAD
                   + j * BLOCK_DIM_LD / VEC_DIM_LD + i] = val[i];
            }
        }
    }
...
#pragma unroll
            for (int mm = 0; mm < VEC_DIM_M; mm++) {
                if constexpr (std::is_same_v<T, half>) {
                    a_reg[mm] = __half2float(
                        a_sm[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_M + PAD_SIZE) + thread_x + mm * BLOCK_DIM_M /
                             VEC_DIM_M]);
                } else if constexpr (std::is_same_v<T, float>) {
                    a_reg[mm] = (a_sm[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_M + PAD_SIZE) + thread_x + mm * BLOCK_DIM_M /
                                      VEC_DIM_M]);
                }
#pragma unroll
                for (int nn = 0; nn < VEC_DIM_N; nn++) {
                    if constexpr (std::is_same_v<T, half>) {
                        b_reg[nn] = __half2float(
                            b_sm[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_N + PAD_SIZE) + thread_y + nn * BLOCK_DIM_N /
                                 VEC_DIM_N]);
                    } else if constexpr (std::is_same_v<T, float>) {
                        b_reg[nn] = (b_sm[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_N + PAD_SIZE) + thread_y + nn *
...
#pragma unroll
            for (int mm = 0; mm < VEC_DIM_M / 2; mm++) {
                a_reg[mm] = __half22float2(
                    a_sm_half[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_M + PAD_SIZE) / 2 + thread_x + mm * BLOCK_DIM_M /
                              VEC_DIM_M]);
#pragma unroll
                for (int nn = 0; nn < VEC_DIM_N / 2; nn++) {
                    b_reg[nn] = __half22float2(
                        b_sm_half[(kk * VEC_DIM_K + kkk) * (BLOCK_DIM_N + PAD_SIZE) / 2 + thread_y + nn * BLOCK_DIM_N /
                                  VEC_DIM_N]);
...
#pragma unroll
    for (int k = 0; k < K; k += BLOCK_DIM_K) {
        ...
        load_tile_n<T, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE>(b_ld, K, thread_x, thread_y,
            start_n, k, b, &b_sm[0][0]);
        __syncthreads();
        // if (k == 0 && block_x == 0 && block_y == 0 && thread_x == 0 && thread_y == 0) {
        ...
        // }
        if constexpr (std::is_same_v<T, half>) {
            compute_tile_vec<T, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE>(
                thread_x, thread_y, &a_sm[0][0], &b_sm[0][0], &c_reg[0]);
        } else if constexpr (std::is_same_v<T, float>) {
            compute_tile<T, VEC_DIM_M, VEC_DIM_N, VEC_DIM_K, BLOCK_DIM_M, BLOCK_DIM_N, BLOCK_DIM_K, PAD_SIZE>(
                thread_x, thread_y, &a_sm[0][0], &b_sm[0][0], &c_reg[0]);
