...
//             int offset = sm_col * (BLOCK_DIM_LD + PAD_SIZE) + sm_row;
// #ifdef _SWIZZLE
...
//     const int BLOCK_DIM_LD, const int BLOCK_DIM_K, const int ELEMENTS_PER_LOAD, const int B, const int MBit, const int
//     S, const int PAD_SIZE>
// __device__ __forceinline__ void load_tile_kv_t(const int ld, const int dim,
//                                                const int thread_x, const int warp_id,
//                                                const int start_m,
//                                                const int k,
//                                                const T *__restrict__ global_mem,
//                                                half *sm) {
//     half *sm_ptr = reinterpret_cast<half *>(sm);
// #pragma unroll
//     for (int j = 0; j < VEC_DIM_LD; ++j) {
//         const int dim0_base = start_m + warp_id + j * (BLOCK_DIM_LD / VEC_DIM_LD);
//         for (int kk = 0; kk < VEC_DIM_K / ELEMENTS_PER_LOAD; ++kk) {
...
//
//             int sm_row = warp_id + j * (BLOCK_DIM_LD / VEC_DIM_LD);
//             int sm_col = thread_x * ELEMENTS_PER_LOAD + kk * (BLOCK_DIM_K / VEC_DIM_K) * ELEMENTS_PER_LOAD;
//             int offset = sm_col * (BLOCK_DIM_N + PAD_SIZE) + sm_row;
// #ifdef _SWIZZLE
//             int addr = swizzle<B, MBit, S>(offset);
// #else
//             int addr = offset; //
...
//
//                     sm_ptr[addr] = val[0].x;
//                     sm_ptr[addr + 1 * (BLOCK_DIM_N + PAD_SIZE)] = val[0].y;
//
//                 }
//             } else {
// #pragma unroll
//                 for (int i = 0; i < 2; ++i) {
//                     sm_ptr[addr + i * (BLOCK_DIM_N + PAD_SIZE)] = 0;
//                     sm_ptr[addr + i * (BLOCK_DIM_N + PAD_SIZE)] = 0;
//                 }
//             }
//         }
...
//     half *k_sm_ptr = reinterpret_cast<half *>(b_sm);
// #pragma unroll
//     for (int head_index = 0; head_index < HEAD_NUM_PER_BLOCK; head_index++) {
...
//                     int k_index_0 = col_base * 2 % (HIDDEN_DIM) * (BLOCK_DIM_N + PAD_SIZE_KV) + n_row_base;
//                     int k_index_1 = (col_base * 2 + 1) % (HIDDEN_DIM) * (BLOCK_DIM_N + PAD_SIZE_KV) + n_row_base;
//                     const float mask_value = __ldg(&mask[(start_m + m_row_base) * N + start_n + n_row_base]);
// #ifdef _SWIZZLE
//                     int k_addr = swizzle<B, MBit, S>(k_index);
// #else
//                     int k_addr_0 = k_index_0;
//                     int k_addr_1 = k_index_1;
// #endif
//
//                     float k_rot_0 = (k_sm_ptr[k_addr_0]);
//                     float k_rot_1 = (k_sm_ptr[k_addr_1]);
//                     c_reg[mm * reg_stride + nn + head_index * VEC_DIM_N] += fmaf(
//                         q_rot.x, k_rot_0, fmaf(q_rot.y, k_rot_1,
//                                                mask_value));
//                 }
//             }
//         }
...
//     const int m_stride = BLOCK_DIM_M / VEC_DIM_M;
//     const int reg_stride = VEC_DIM_N * HEAD_NUM_PER_BLOCK;
//     half *v_sm_ptr = reinterpret_cast<half *>(sm);
// #pragma unroll
//     for (int head_index = 0; head_index < HEAD_NUM_PER_BLOCK; head_index++) {
// #pragma unroll
//         for (int mm = 0; mm < VEC_DIM_M; mm++) {
...
//                     int index_0 = kk * 2 % (HIDDEN_DIM) * (BLOCK_DIM_N + PAD_SIZE_KV) + n_row_base;
//                     int index_1 = (kk * 2 + 1) % (HIDDEN_DIM) * (BLOCK_DIM_N + PAD_SIZE_KV) + n_row_base;
//
// #ifdef _SWIZZLE
//                     int addr = swizzle<B, MBit, S>(index_0);
// #else
//                     int addr_0 = index_0;
//                     int addr_1 = index_1;
// #endif
//                     float v_reg_0 = (v_sm_ptr[addr_0]);
//                     float v_reg_1 = (v_sm_ptr[addr_1]);
//
//                     acc.x = fmaf(v_reg_0, c_reg[mm * reg_stride + nn + head_index * VEC_DIM_N], acc.x);
//                     acc.y = fmaf(v_reg_1, c_reg[mm * reg_stride + nn + head_index * VEC_DIM_N], acc.y);
//                 }
// #pragma unroll
//                 for (int offset = 16; offset > 0; offset /= 2) {
...
//
//     constexpr int shared_mem_block_size = (BLOCK_DIM_K_KV) * (BLOCK_DIM_N + PAD_SIZE_KV);
...
//
//     int flip_flag = 0;
//     load_tile_kv_t<T_KV, VEC_DIM_N, VEC_DIM_K_KV, BLOCK_DIM_N, BLOCK_DIM_K_KV, ELEMENTS_PER_LOAD_KV, B, MBit, S,
//         PAD_SIZE_KV>(
...
//     __syncthreads();
//
//     // half *k_sm_ptr = reinterpret_cast<half *>(&sm[(flip_flag) * shared_mem_block_size]);
//     // for (int j = 0; j < BLOCK_DIM_K_KV; j++) {
//     //     for (int i = 0; i < BLOCK_DIM_N; i++) {
//     //         if (thread_x == 0 && warp_id == 0 && j == 0 && blockIdx.y == 0) {
//     //             printf("k: %d, n: %d, k_sm_ptr[%d]: %lf\n",
//     //                    j, i, j * (BLOCK_DIM_N + 2 * PAD_SIZE_KV) + i,
//     //                    __half2float(k_sm_ptr[j * (BLOCK_DIM_N + 2 * PAD_SIZE_KV) + i]));
//     //         }
//     //     }
//     // }
//
...
//         if (next_n < N) {
//             //
//             load_tile_kv_t<T_KV, VEC_DIM_N, VEC_DIM_K_KV, BLOCK_DIM_N, BLOCK_DIM_K_KV, ELEMENTS_PER_LOAD_KV, B, MBit,
//                 S, PAD_SIZE_KV>(
